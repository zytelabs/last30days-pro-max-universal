...
The pattern of summarizing recent public signal is not new. This engine is directly inspired by Matt van Horn's [last30days skill](https://github.com/mvanhorn/last30days-skill), an agent skill that researches a topic by aggregating recent signal across a dozen social platforms and prediction markets, and it keeps that project's core insight while deliberately swapping the spine. Where the original maintains a separate credentialed connector for every platform, this version leans on a single web-data layer for discovery, which is the change that makes it general enough to point at any topic. The catch with the many-connectors approach is that each platform wants its own login, its own API key, its own rate limit, and its own brittle scraping workaround the week it changes its markup, so you end up spending your time maintaining connectors instead of doing research.
